...
for j = 1:1
    ...
    step = 1;
    disp("========================================");
    disp("Iteration: "+1+" Smooth loss = " + smoothloss);
    disp("========================================");
    ...
    for i = 1:floor(N*RNN.hyperparams.N_EPOCHS/RNN.hyperparams.seq_length)
        ...
        if mod(step,10000) == 0
            disp("========================================");
            disp("Iteration: "+step+" Smooth loss = " + smoothloss);
            disp("========================================");
            str1hot = generateMessage(RNN,trainX(:, 1) ,h,200);
            message = indices2string(str1hot,ind_to_char);
            disp(message);
...
[bookdata, char_to_ind, ind_to_char,N, RNN] = initModel();
h = RNN.h0;
X = bookdata(1:RNN.hyperparams.seq_length);
Y = bookdata(2:RNN.hyperparams.seq_length+1);
X = string2indices(X,char_to_ind);
Y = string2indices(Y,char_to_ind);
ForwardPassVals = forwardpasscalc(RNN,RNN.h0,X,Y);
h = 1e-4;
grads = backwardsprop(RNN,ForwardPassVals,X);
...
grad = [gradb;gradc;gradU;gradW;gradV];
numgrad = [numgradb;numgradc;numgradU;numgradW;numgradV];

verification = norm(grad-numgrad,1)/max(1e-6,norm(grad,1)+norm(numgrad,1));
...
function [bookdata,char_to_ind,ind_to_char] = loadDataSet()
    ...
    char_to_ind = containers.Map(characters,linspace(1,K,K));
    ind_to_char = containers.Map(linspace(1,K,K),characters);
    bookdata = book_data;
end
...
function num_grads = ComputeGradsNum(X, Y, RNN, hstep)

for f = fieldnames(RNN)'
    disp('Computing numerical gradient for')
    disp(['Field name: ' f{1} ]);
    num_grads.(f{1}) = ComputeGradNum(X, Y, f{1}, RNN, hstep);
end
